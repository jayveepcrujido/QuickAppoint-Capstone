<?php
/**
 * UMID / SSS ID Extractor
 * Extracts data from UMID or SSS ID card
 */

function extract_data_umid($ocrClean, $ocrUpper) {
    $data = [
        'first_name' => '',
        'middle_name' => '',
        'last_name' => '',
    ...
    // NAME EXTRACTION
    // ===================================
    // UMID format typically has labeled fields
    
    // Method 1: Look for "Last Name" or "Surname"
    if (preg_match('/(?:Last\s+Name|Surname)[:\s]*([A-Z][A-Za-z\s\-\']+)/i', $ocrClean, $m)) {
    ...
    // Method 2: Look for "First Name" or "Given Name"
    if (preg_match('/(?:First\s+Name|Given\s+Name)[:\s]*([A-Z][A-Za-z\s\-\']+)/i', $ocrClean, $m)) {
        $candidate = ucwords(strtolower(trim(preg_replace('/[^A-Za-z\-\s]/', '', $m[1]))));
        if (is_valid_name($candidate) && strlen($candidate) >= 3) {
    ...
    // Method 4: Fallback - comma-separated format "LASTNAME, FIRSTNAME MIDDLENAME"
    ...
    // Method 1: Look for "Date of Birth" or "Birthday" label
    ...
    // Method 2: Fallback - any date pattern YYYY-MM-DD or YYYY/MM/DD
    ...
    // Method 3: Additional fallback - MM/DD/YYYY format
    ...
    // Method 1: Look for "Sex" label with full word
    if (preg_match('/Sex[:\s]*(Male|Female)\b/i', $ocrClean, $sMatch)) {
        $sex = ucfirst(strtolower($sMatch[1]));
        $data['sex'] = $sex;
    }
    
    // Method 2: Look for "Sex" label with M/F
    if (empty($data['sex']) && preg_match('/Sex[:\s]*([MF])\b/i', $ocrUpper, $sMatch2)) {
        $data['sex'] = ($sMatch2[1] === 'M') ? 'Male' : 'Female';
    }
    
    // Method 3: Look for standalone M or F near other personal info
    ...
    // Method 2: Look for common Philippine address patterns
    if (empty($data['address']) && preg_match('/([^\n]{15,200}(?:QUEZON|MANILA|CITY|BARANGAY|BRGY)[^\n]{0,50})/i', $ocrClean, $aMatch2)) {
        $addrRaw = $aMatch2[1];
        $addrClean = preg_replace('/[^\w\s,\.\-]/', ' ', $addrRaw);
        $addrClean = preg_replace('/\s+/', ' ', $addrClean);
        $addrClean = trim($addrClean);
